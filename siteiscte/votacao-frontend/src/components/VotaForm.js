  ...
  votaEFechaModal = (e) => {
    ...
    for (let i = 0; i < this.props.opcoes.length; i++) {
      if (this.props.opcoes[i].pk == this.state.selectedOption) {
        axios.put(API_URL_OPCOES + this.props.opcoes[i].pk, {
          pk: this.props.opcoes[i].pk,
          questao: this.props.opcoes[i].questao,
          opcao_texto: this.props.opcoes[i].opcao_texto,
          votos: this.props.opcoes[i].votos++,
        });
      }
    }
    this.props.toggle();
