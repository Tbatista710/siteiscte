  ...
  render() {
    return (
      <Form onSubmit={this.fechaModalQuestao}>
            ...
            <tbody>
              {
                !this.state.opcao_set || this.state.opcao_set.length <= 0 ? (
                  ...
                  // map through the opcao_set array to render each row
                  this.state.opcao_set.map((opcao, index) => (
                    <tr key={opcao.pk}>
                      <td colSpan="6" align="left">
                        {this.defaultIfEmpty(opcao.opcao_texto)}
                      </td>
                      <td colSpan="6" align="right">
                        {opcao.votos}
                      </td>
                      <td>
                        <Button color="primary" size="sm" onClick={() => this.props.abreModalOpcao(index)}>
                          Editar
                        </Button>
                      </td>
                    </tr>
        ...
        <Button color="danger" onClick={this.fechaModalQuestao}>
          Fechar
        </Button>
      </Form>
    );
